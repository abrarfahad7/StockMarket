...
def buy():
    ...
    if request.method == "POST":
        missing_errors = symbol_given() or shares_given()
        if missing_errors:
            return missing_errors
        elif not request.form.get("shares").isdigit():
            return apology("shares can only be whole numbers")
...
def login():
    ...
    if request.method == "POST":
        ...
        check_results = username_given() or password_given()
...
def register():
    """Register user"""
    if request.method == "POST":
        check_results = username_given() or password_given() or rePass_given()
        if check_results != None:
            return check_results

        if request.form.get("password") != request.form.get("confirm"):
            return apology("Both passwords need to match")
        ...
        if prim_key is None:
            return apology("Registration Error. Check if username exists.", 403)
